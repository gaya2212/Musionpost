...
create table public.verified_credentials (
  ...
  issuer_signature text,          -- nullable for MVP
  c2pa_manifest_url text,         -- nullable for MVP
  created_at timestamptz not null default now()
);
...
-- ------------------- projects -------------------
create policy "projects: artist owner reads own"
  on public.projects for select
  using (auth.uid() = artist_profile_id);

create policy "projects: accepted collaborators read"
  on public.projects for select
  using (
    exists (
...
create policy "projects: public visibility readable"
  on public.projects for select
  using (visibility = 'public');

create policy "projects: artist owner manages own"
  on public.projects for all
  using (auth.uid() = artist_profile_id)
  with check (auth.uid() = artist_profile_id);

-- ------------------- project_stages -------------------
create policy "project_stages: same access as parent project"
...
create policy "project_stages: artist owner manages"
...
-- ------------------- project_collaborators -------------------
create policy "collaborators: readable by project owner and involved pro"
...
create policy "collaborators: artist manages invites"
...
create policy "collaborators: pro can update own status"
  on public.project_collaborators for update
  using (pro_profile_id = auth.uid());

create policy "collaborators: artist can update collaborators"
  on public.project_collaborators for update
  using (
    exists (
      select 1 from public.projects p
...
-- ------------------- matches -------------------
create policy "matches: only project owner reads"
...
create policy "matches: project owner updates decisions"
...
-- Matches are inserted server-side via service role only.
...
create policy "thread_participants: readable by co-participants"
...
create policy "credentials: project owner reads"
...
create policy "credentials: contributors read"
  on public.verified_credentials for select
  using (
    exists (
      select 1 from public.project_collaborators pc
      where pc.project_id = verified_credentials.project_id
      and pc.pro_profile_id = auth.uid()
      and pc.status in ('accepted', 'completed')
    )
  );

-- Credentials are issued server-side via service role only.

-- ------------------- credits_export -------------------
create policy "credits_export: project owner reads"
...
-- Credits exports are written server-side via service role only.

-- ============================================================================
-- Trigger: update updated_at on row updates
-- ============================================================================
